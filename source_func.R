# --------------------------------------------------------------------------------------
# description: model confidence boounds for high-dimensional variable selection
# author: Xiaonan Hu
# date: 2022/07/11
# revision: 
# ---------------------------------------------------------------------------------------
...
mcb_candidate <- function(var.matrix, p, r) # PI-MCB algorithm for computing MCBs candidate
{
  #--------------------
...
get_opt_mcb_binary_search <- function(var.matrix, confidence.level, p, r) # BIS-MCB for computing optimal MCB
{
  ## aims to find the final width by binary search, and then get the optimal mcb under the width
  ...
  iter_width<-c()
  i<-1
  width.lower<-c();width.lower[1]<-0 # width search lower bound
  width.iter<-c();width.iter[1]<-floor(p/2) # current width
  width.upper<-c();width.upper[1]<-p # width search upper bound
  oper_times<-0
  
  while((width.upper[i]-width.lower[i])>0){
    # fixed width in each round
    cap<-vector(length=p-width.iter[i]+1);cap[1]<-0  
    for(j in 0:(p-width.iter[i])) # j is the length of lowerbound
    {
    ...
    # shrinkage the width (bisection)
    if((width.upper[i]-width.lower[i])==1)  # case: upper=5, lower=4, width_iter=4
    { 
      if(freq[i]>=confidence.level) # choose lower=width_iter as optimal width
      {
      ...
      }else{ # choose upper=width_iter+1 as optimal width
        if(width.upper[i]==p){
...
alter_model <- function(datasets, p, method='scaledlasso') 
{
  ...
  if(method=='scaledlasso'){
    ...
    warning("Methods is misspecified!!")
  }
